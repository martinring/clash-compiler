...
module CLaSH.Backend.Yices.Driver where
...
generateYices :: forall backend . Backend backend
              ...
              -> IO ()
generateYices bindingsMap yicesState primMap tcm tupTcm typeTrans eval (topEntity,annM) testInpM expOutM opts (startTime,prepTime) = do
  ...
  let modName   = takeWhile (/= '.') (name2String topEntity)
      ...
      yicesState' = setModName modName
                $ fromMaybe (initBackend iw hdlsyn :: backend) yicesState
      mkId      = evalState mkBasicId yicesState'
  ...
  let topWrapper = mkTopWrapper primMap' mkId annM modName iw (snd topComponent)
      hdlDocs = createYices yicesState' modName ((noSrcSpan,topWrapper) : netlist ++ testBench)
      dir = fromMaybe "." (opt_hdlDir opts) </>
            CLaSH.Backend.name yicesState' </>
            takeWhile (/= '.') (name2String topEntity)
  prepareDir (opt_cleanhdl opts) (extension yicesState') dir
...
sortComponents cs
  ...
  | otherwise = ok ++ sortComponents rs
    where cs' = map (componentName . snd) cs
          (ok,rs) = partition (null . intersect cs' . nestedComponents . snd) cs
...
nestedComponents c = insts ++ bb
  where
    ...
    bb = do
      decl <- declarations c
      case decl of
        BlackBoxD _ _ _ _ _ ctx -> do
           f <- map fst $ IntMap.elems (bbFunctions ctx)
           case f of
            Right (InstDecl name _ _) -> [name]
            _ -> []
        _ -> []
...
createYices :: Backend backend
           => backend     -- ^ Backend
           -> String
           -> [(SrcSpan,Component)] -- ^ List of components
           -> [(String,Doc)]
createYices backend modName components = flip evalState backend $ do
  -- (hdlNms,hdlDocs) <- unzip <$> mapM genHDL components
  -- let hdlNmDocs = zip hdlNms hdlDocs
  (hdlNmDocs,incs) <- unzip <$> mapM (uncurry (genHDL modName)) (sortComponents components)
